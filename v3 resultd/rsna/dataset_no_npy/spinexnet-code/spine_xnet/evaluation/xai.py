...
def attention_rollout(
    ...
    for i, block in enumerate(blocks):
        ...
        def make_new_forward(original_fwd, idx):
            def new_forward(self_attn, x, attn_mask=None, **kwargs):
                B, N, C = x.shape
                ...
                if attn_mask is not None:
                    if attn_mask.ndim == 2:
                        ...
                        attn = attn + attn_mask.to(dtype=attn.dtype, device=attn.device).unsqueeze(1)
    ...
    seq_len = result.shape[-1] - num_prefix_tokens
    grid_size = int(seq_len ** 0.5)
    if grid_size * grid_size != seq_len:
        raise RuntimeError(
            f"Cannot reshape attention rollout tokens into a square grid: "
            f"seq_len={seq_len}, prefix_tokens={num_prefix_tokens}"
        )
    rollout_map = result[0, num_prefix_tokens:].reshape(grid_size, grid_size)
