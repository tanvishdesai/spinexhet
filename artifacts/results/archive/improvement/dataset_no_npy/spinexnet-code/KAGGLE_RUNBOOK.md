...
```
  ...
  [DONE]           ├─ S3a: CBM Non-Leaky ─────┤      ↓
                    ...
                    ├─ S4a: ResNet50 ──────────┤      ↓
                    ├─ S4b: DenseNet121 ───────┤   S7 (GLS + Randomization)
                    ├─ S4c: EfficientNet-B4 ───┤      ↓
...
| **Group E** | S7a (GLS), S7b (Randomization) | Yes × 2 | ~2h each |
...
## Session 7 — Gradient Locality Score + Model Randomization

**Purpose:** Tier 2 (GLS analysis) and Tier 3 (randomization sanity check).

### 7a: Gradient Locality Score

```bash
!python $CODE/scripts/gradient_locality.py \
  --configs \
  ...
  --names resnet50 densenet121 convnext_tiny efficientnet_b4 vit_small \
  --manifest $MANIFEST --fold 0 --cache-dir $CACHE \
...
| **GLS scatter plot** | `gls_vs_agreement_scatter.png` | **Figure 5** |
...
| # | Config File | Model Type | Purpose | Folds |
|---|-------------|------------|---------|-------|
| 1 | `convnext_blackbox.yaml` | Black-box | Primary baseline | 0,1,2 |
| 2 | `cbm_nonleaky.yaml` | CBM (7 anatomy concepts) | Primary interpretable | 0 |
| 3 | `cbm_visual.yaml` | CBM (10 BiomedCLIP concepts) | **NEW** visual CBM | 0 |
| 4 | `cbm_leaky.yaml` | CBM (label-leaking) | Leakage ablation | 0 |
| 5 | `resnet50.yaml` | Black-box | Architecture diversity | 0 |
| 6 | `densenet121.yaml` | Black-box | Architecture diversity | 0 |
| 7 | `efficientnet_b4.yaml` | Black-box | Architecture diversity | 0 |
| 8 | `vit_small.yaml` | Black-box (ViT) | CNN vs Transformer | 0 |
...
| **Table 7: Gradient Locality Scores** | `gradient_locality_scores.csv` | `gradient_locality.py` |
...
| **Figure 3: GLS vs agreement scatter** | GLS + agreement CSVs | `gradient_locality.py` |
...
2. **Session 7a** (one notebook, ~2h): Gradient Locality Score on all 5 black-box models
...
4. Session 7 (1 notebook, ~3h) — GLS + randomization
