...
- For Tier 1 multi-fold, train folds 1 and 2 for every model you report in the
  classification table. Fold 0 already exists.
- For XAI stability, run the XAI benchmark on folds 0 and 1 at minimum. Use
  folds 0, 1, and 2 only if you want to claim XAI metrics are averaged over all
  three folds.
...
## Cell 2: Evaluate folds 0 to 2 and create the classification table

Run this after fold 1 and fold 2 checkpoints are available in `/kaggle/working`
or attached as an input dataset.
...
## Cell 3: XAI benchmark v2 on folds 0 and 1

Use `XAI_FOLDS = [0, 1, 2]` only if you want the manuscript to say XAI metrics
are averaged over folds 0 to 2.
...
## Cell 5: Gradient Locality Score

```python
GLS_MODELS = ["resnet50", "densenet121", "convnext_blackbox", "efficientnet_b4", "vit_small"]
fold = 0

run([
    sys.executable, CODE / "scripts/gradient_locality.py",
    "--configs", *[CFG[m] for m in GLS_MODELS],
    "--checkpoints", *[find_checkpoint(m, fold) for m in GLS_MODELS],
    "--names", *GLS_MODELS,
    "--manifest", MANIFEST,
    "--fold", fold,
    "--cache-dir", CACHE,
    "--output-dir", WORK / "gradient_locality",
    "--max-samples", 300,
